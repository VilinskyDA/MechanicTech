...
## 5. Core loop
1) Сборка оружия/брони/снаряжения из модулей.
2) Валидация + предрасчёт производных параметров.
3) Подготовка бойца/отряда (слоты, масса, боекомплект, энергия).
4) Пошаговый бой (AP, укрытия, видимость, применение эффекторов).
5) Пост-бой: износ/ремонт/расходники → новая итерация сборки.

---

## 6. Конструктор (KSP-подобная сборка)
### 8.1 Термины
...
### 8.2 Базовые роли (для классификации и UI)
...
### 8.5 Предрасчёт физики на этапе валидации
...
## 7. Табличные модели воздействия
...
## 8. Контракт воздействия: Impact Packet и Threat Model
### 8.1 Пакет воздействия и модель угроз
...
### 8.2 Impact Packet (общий контракт)
...
### 8.3 Threat Model и слои защиты
...
## 9. Carrier State и распространение в пространстве
...
## 10. Барьеры окружения: геометрия, слои, transfer function
### 14.1 Представление барьера
...
### 14.2 Эффективная толщина и режим контакта
...
### 14.5 Предрасчитанные таблицы (response surfaces)
...
### 14.1 Поля кинетического канала (Impact Packet)
...
## 13. Пошаговый бой (использует результаты предрасчёта)
### 14.1 Общие принципы
...
## 14. Данные, тесты и воспроизводимость
...
- Валидация + генерация таблиц `H` и `G`.
