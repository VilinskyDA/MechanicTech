...
>
> **Примечание о формате:** данный документ совмещает дизайнерское видение (GDD) и формальные технические контракты (Tech Spec): операторы `F_layer`, `H`, `G`, структуры данных Impact Packet, MaterialDefinition и т.д. По мере роста проекта формальные контракты данных и API будут вынесены в отдельные технические документы; этот файл останется основным GDD.
...
### 14.2 Индивидуальные параметры бойца
...
### 14.3 Взаимодействие персонажа и снаряжения
...
## 15. Пошаговый бой (использует результаты предрасчёта)
...
Базовый бюджет AP определяется выносливостью бойца (раздел 14.2). Масса экипировки, утомление и ранения снижают доступные AP.
...
### 15.6 Взаимодействие с окружением
...
### 16.2 Физические ограничения (естественные trade-off'ы)
...
### 16.3 Экономические ограничения
...
## 17. Экономика и прогрессия (концепция)

### 17.1 Источники модулей и материалов
...
## 18. Данные, тесты и воспроизводимость
...
### 19.2 Slice 1 — Сборка и взаимодействие
...
### 19.3 Slice 2 — Playable Prototype
...
## 20. Дорожная карта (приоритизация разделов по этапам)

Привязка систем документа к этапам прототипа (раздел 19):

### Slice 0 — Proof of Pipeline
| Раздел | Что нужно |
|--------|-----------|
| 9. Контракт воздействия | Impact Packet: минимальный кинетический канал |
| 10. Carrier State | Один профиль распространения (пуля) |
| 11. Барьеры окружения | Один барьер, одна transfer function H |
| 12. Библиотека материалов | 1–2 материала (металл), минимальная MaterialDefinition |
| 13. Threat Model v0.1 | Только кинетический канал |
| 18. Данные и тесты | Golden-тест на результат одного выстрела |

### Slice 1 — Сборка и взаимодействие
| Раздел | Что нужно |
|--------|-----------|
| 7. Конструктор | Полный граф сборки: узлы, валидация, предрасчёт |
| 8. Табличные модели | Генерация таблиц H и G для кинетики |
| 12. Библиотека материалов | 3–4 материала, LayerDefinition, законы материала |
| 13. Threat Model v0.1 | Кинетика + базовый термо |
| 16. Баланс | Износ, деградация, стохастика |
| 18. Данные и тесты | Серия golden-тестов, детерминированный seed |

### Slice 2 — Playable Prototype
| Раздел | Что нужно |
|--------|-----------|
| 5. Управление сложностью | Базовые пресеты сборок |
| 6. Core loop | Полный цикл: сборка → бой → пост-бой |
| 11. Барьеры окружения | Укрытия с деградацией, несколько типов |
| 12. Библиотека материалов | Полный набор v0.1 (металл/дерево/бетон/стекло/грунт) |
| 14. Модель персонажа | Базовые параметры, ранения по зонам |
| 15. Пошаговый бой | Тайловая карта, AP, видимость, overwatch, условия победы |
| 15.8 ИИ | Базовый ИИ (укрытия, стрельба) |

### Slice 3 — Vertical Slice
| Раздел | Что нужно |
|--------|-----------|
| 2. Видение | Сеттинг: нарратив технологического отката |
| 5. Управление сложностью | LOD вкладки, раскрытие параметров с прогрессией |
| 6. Core loop (6.3) | Post-battle отчёт, все три уровня обратной связи |
| 14. Модель персонажа | Рост параметров с опытом |
| 16. Баланс | Полная экономика AP/масса/энергия/износ |
| 17. Экономика | Реверс-инжиниринг, крафт, торговля |
